...
contract marketplace is Ownable {
    ...
    function addPaymentToken (address paymentToken_) external onlyOwner {
        require (
            paymentToken_ != address(0),"NFT Marketplace paymentToken_ is zero address"
        );
        require (
    ...
    modifier onlySupportedPaymentToken (address paymentToken_) {
        require (
            isPaymentTokenSupported(paymentToken_),"NFTMarketplace: unsuport payment token"
        );
        _;
    }
    ...
    function cancelOrder(uint256 orderId_) external {
        Order storage _order = orders[orderId_];
        require (
            _order.buyer == address(0),"NFTMarketplace: buyer must be zero"
        );
        require (
            _order.seller == _msgSender(), "NFTMarketplace: must be owner"
        );
    ...
    function getOrderInfo (uint orderId_) public view returns(address,uint256,address,uint256) {
        Order storage _order = orders[orderId_];
        return (_order.seller, _order.tokenId, _order.paymentToken, _order.price); 
    }
