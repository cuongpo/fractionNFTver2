...
contract StakingRewards {
    ...
    function stake(uint256 tokenId_,uint _index) external updateReward(_index,msg.sender){
        ...
        require(
            nftcontract(nftContract).getrealEstateId(tokenId_) == pools[_index].realEstateId,
    ...
    function setRewardsDuration(uint _duration,uint _index) external {
        // Code
        require (nftcontract(nftContract).getrealEstateOwner(pools[_index].realEstateId)==msg.sender,"not realEstate owner");
    ...
    function getPoolIdByrealEstateId(uint256 realEstateId) public view returns (bool check,uint256 poolId) {
